...
The seed creates seven fixed sign-in accounts, one per role. The password for all of them is
`Demo@1234`. The login page lists them as clickable buttons — pick one and press **Sign in**.
...
The seed also generates ~36 staff accounts with randomly generated Persian names at
`@kavir-grid.demo` (same password). Those populate realistic project-manager and engineer
assignments — their addresses change on every seed run, so use the fixed accounts above to sign in.

> **If every account is rejected with "Invalid credentials",** the demo data has not been seeded.
> Run `npm run db:seed` (or `docker compose run --rm migrate`) and try again. A database seeded with
> a build older than this one will not contain the fixed accounts; re-running the seed creates them.
...
### Seeding fails with `Cannot find module '@simorgh/shared/dist/index.js'`

The shared package's entry point is `dist/index.js`, and the seed script imports it. If that build
is missing the seed cannot start. Migrations still succeed, because `migrate.ts` does not import the
shared package — which is why this surfaces only at the seed step.

Build it and re-run:

```bash
npm run build --workspace=@simorgh/shared
npm run db:seed
```

`npm install` builds it automatically (via the package's `prepare` script), so this normally only
appears if the install was run with `--ignore-scripts` or was interrupted.
